...
/*
 * 在LCD屏幕上显示图片(数据水平字节垂直)
 * @param x 图片左上角的x坐标
 * @param y 图片左上角的y坐标
 * @param px 图片的宽度（8像素整数倍）
 * @param py 图片的高度（8像素整数倍）
 */
void lcd_show_picture(unsigned short x, unsigned short y, unsigned short px, unsigned short py, unsigned char gImage_test[])
{
	unsigned char i, j, temp, t1;
	for (i = 0; i < py / 8; i++)
	{
		for (j = 0; j < px; j++)
		{
			temp = gImage_test[i * px + j];
			for (t1 = 0; t1 < 8; t1++)
			{
				if (temp & 0x80)
					lcd_drawpoint(x + j, y + i * 8 + t1, tftlcd_dev.forecolor);
				else
				{
					lcd_drawpoint(x + j, y + i * 8 + t1, tftlcd_dev.backcolor);
				}
...
// px - 宽度
// py - 高度
void lcd_clear_picture(unsigned short x, unsigned short y, unsigned short px, unsigned short py)
{
	unsigned char i, j;
	for (i = 0; i < py; i++)
	{
		for (j = 0; j < px; j++)
		{
			lcd_drawpoint(x + j, y + i, tftlcd_dev.backcolor);
		}
	}
}
